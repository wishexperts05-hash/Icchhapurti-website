...
export default function RefferalPaymentModal({
    ...
    productId, productName, price, productImg, refferalCode,
}) {
    ...
    useEffect(() => {
        ...
        const products = [{
            _id: productId,
            productId,
            product: { _id: productId, name: productName, price, image: productImg },
            quantity: 1,
            totalAmount: price
        }];
    ...
    }, [isAuthenticated, cartItems, addresses, couponCode, referralCode, addressIndex]);
    ...
    useEffect(() => { setCurrentStep(addresses.length > 0 ? 2 : 1); }, [addresses]);
    ...
    const fetchBalance = async () => {
        try {
            ...
            const balanceRes = await fetch(`${import.meta.env.VITE_API_URL}/api/user/walletAndTransaction/getWalletBalance`, { headers });
    ...
    const fetchAddresses = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/user/address/getAllAddress`, {
    ...
    const checkPincodeServiceability = async (pinCode) => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/user/shipping/check-pincode/${pinCode}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}`, 'Content-Type': 'application/json' },
            });
            const data = await res.json();
    ...
    const fetchCheckOutDetails = async () => {
        try {
            ...
            const res = await fetch(
                ...
                { method: 'POST', headers: { Authorization: `Bearer ${localStorage.getItem('token')}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }
            );
            const data = await res.json();
            if (data.success) {
    ...
    const verifyRazorpayPayment = async (paymentData) => {
        ...
        } catch (error) { console.error('Payment verification failed:', error); return false; }
    };
    ...
    const handleRazorpayPayment = useCallback(async () => {
        ...
        razorpay.open();
    }, [orderToken, addresses, addressIndex, user, Navigate, onClose, scrollToTop, setCount]);
    ...
    const fetchOffers = async () => {
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/user/orders/offers`, {
                method: 'POST',
    ...
    const fetchEstimatedDelivery = async () => {
        ...
        } catch (error) { console.error('Failed to fetch EDD:', error); }
    };
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
                <div className="bg-white rounded-xl p-5 w-full max-w-sm shadow-lg text-center">
                    ...
                    <div className="flex gap-3 justify-center">
                        <button onClick={initializePaymentPage} className="px-4 py-2 text-sm font-medium text-white bg-red-500 hover:bg-red-600 rounded-lg transition">Try Again</button>
                        <button onClick={() => { onClose(); Navigate('/cart'); }} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">Back to Cart</button>
                    </div>
                </div>
            </div>
        );
    ...
    /* ── Render ── */
    return (
        <div className="fixed inset-0 z-[9999] h-screen scroll-smooth backdrop-blur-[2px] bg-black/20">
            ...
            {(initialLoading || checkoutLoading) && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative z-10 bg-white px-6 py-4 rounded-xl shadow-xl flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-gray-900">{initialLoading ? 'Loading...' : 'Processing...'}</span>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
                        ...
                        <div className="flex gap-3 mt-6">
                            <button onClick={() => setWarning(false)} className="flex-1 px-4 py-2.5 bg-purple-900 cursor-pointer text-white rounded-lg font-medium hover:bg-purple-800 transition-colors">
                                Continue Checkout
                            </button>
                            <button
                                onClick={() => { cancelledOrder(); onClose(); }}
                            ...
                            <div className="bg-white rounded-lg px-1 shadow-sm overflow-hidden">
                                ...
                                {((offers?.length ?? 0) > 0 || (spinOffers?.length ?? 0) > 0) && (addresses?.length ?? 0) > 0 && (
                                    <OfferDisplay
                                        ...
                                        couponCode={couponCode} isAuthenticated={isAuthenticated}
                                    />
                                )}
                                ...
                                {isAuthenticated && checkoutDetails && addresses.length > 0 && (
                                    <>
                                        ...
                                        <PaymentMethods paymentMethods={paymentMethods} selectedMethod={selectedMethod} onSelect={setSelectedMethod} />

                                        <PriceBreakdown checkoutDetails={checkoutDetails} totalItems={totalItems} couponDiscount={couponDiscount} />
                                        ...
                                        <p className="text-center text-gray-400 text-sm mt-4">By placing this order, you agree to our terms and conditions</p>

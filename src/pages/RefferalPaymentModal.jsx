...
export default function RefferalPaymentModal({
    ...
    // Referral modal specific success/delivery state
    ...
    }, [isAuthenticated, productId, productName, price, productImg]);

    const initializePaymentPage = async () => {
        setInitialLoading(true);
        setError(null);
        try {
            await Promise.all([payment.fetchBalance(), payment.fetchAddresses()]);
        } catch (err) {
    ...
    useEffect(() => { payment.fetchOffers(); }, [productId]);

    useEffect(() => {
        if (isAuthenticated && cartItems.length > 0 && payment.addresses.length > 0) {
            payment.fetchCheckOutDetails();
        }
    }, [isAuthenticated, cartItems, payment.addresses, couponCode, referralCode, addressIndex]);
    ...
    useEffect(() => { setCurrentStep(payment.addresses.length > 0 ? 2 : 1); }, [payment.addresses]);
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
                <div className="bg-white rounded-xl p-5 w-full max-w-sm shadow-lg text-center">
                    ...
                    <div className="flex gap-3 justify-center">
                        <button onClick={initializePaymentPage} className="px-4 py-2 text-sm font-medium text-white bg-red-500 hover:bg-red-600 rounded-lg transition cursor-pointer">Try Again</button>
                        <button onClick={() => { onClose(); Navigate('/cart'); }} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition cursor-pointer">Back to Cart</button>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="fixed inset-0 z-[9999] h-screen scroll-smooth backdrop-blur-[2px] bg-black/20">
            ...
            <CheckoutOverlays
                ...
                onWarningLeave={() => { payment.cancelledOrder(); onClose(); }}
                    ...
                    <div className="bg-gray-50">
                        <div className="mx-auto px-2 py-6">
                            <div className="bg-white rounded-lg px-1 shadow-sm overflow-hidden">
                                ...
                                <AddressModal
                                    addresses={payment.addresses} fetchAddresses={payment.fetchAddresses}
                                ...
                                {isAuthenticated && auth.authStage === 'checkout' && ((payment.offers?.length ?? 0) > 0 || (payment.spinOffers?.length ?? 0) > 0) && (payment.addresses?.length ?? 0) > 0 && (
                                    <OfferDisplay
                                        offers={payment.offers} spinOffers={payment.spinOffers}
                                ...
                                {isAuthenticated && auth.authStage === 'checkout' && payment.checkoutDetails && payment.addresses.length > 0 && (
                                    <>
                                        ...
                                        <PriceBreakdown checkoutDetails={payment.checkoutDetails} totalItems={totalItems} couponDiscount={payment.couponDiscount} />
